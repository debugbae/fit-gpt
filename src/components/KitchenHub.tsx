      ...
      <header className="px-1 flex items-center justify-between">
        ...
        <div className="flex bg-zinc-900/40 backdrop-blur-sm p-1 rounded-2xl border border-zinc-800/50">
          ...
          <button 
            ...
            className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'shop' ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            Shop
          </button>
          <button 
            onClick={() => setActiveTab('recipes')}
            className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'recipes' ? 'bg-blue-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            Recipes
          </button>
        </div>
              ...
              <div className="space-y-3">
                {shoppingList.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-4 p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-2xl group transition-all hover:border-blue-500/30 backdrop-blur-sm">
                    ...
                    <i className="fas fa-plus-circle ml-auto text-zinc-700 group-hover:text-blue-500"></i>
                  </div>
                ))}
                <div className="pt-6 border-t border-zinc-700/50 mt-4">
                  <button className="w-full py-4 bg-zinc-700 text-zinc-400 font-black uppercase text-[10px] tracking-widest rounded-2xl hover:text-white transition-all">
